...
! tdWriteVector2D_AToMulti
  integer function strlen(st)
      ...
      character		st*(*)
      i = len(st)
      do while (st(i:i) .eq. ' ' .or. ichar(st(i:i)) .eq. 0)
        i = i - 1
      enddo
      strlen = i
      return
...
SUBROUTINE TDI_READ_SCALAR (a_xScalarId, FileUnit, error)
  ...
  INTEGER             iTime
  CHARACTER(LEN=100)   TimeUnits, Units
...
nActive = None
  ...
  WRITE(FileUnit,*) 'Time units: ', TimeUnits(1:strlen(TimeUnits))
  ...
  ! read the units
  call XF_GET_DATASET_UNITS(a_xScalarId, Units, error)
  if (error < 0) return
  WRITE(FileUnit,*) 'Units: ', Units(1:strlen(Units))
